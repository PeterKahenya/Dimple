...
public class DimplePolygon extends Polygon{
    ...
    public DimplePolygon(){
     ...
     private void prepareProps(final DimplePolygon shape) {
        props = new ListView<>();
     ...
     fill.setOnAction(new EventHandler<ActionEvent>() {
      ...
      numSides.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent t) {
               shape.setNumSides(Integer.parseInt(((TextField) t.getSource()).getText()));
            }
        });
        ...
        radius.setOnAction(new EventHandler<ActionEvent>() {
         ...
         props.getItems().addAll(
                new Property(new Label("# of Sides"), numSides),
